...
def get_params():
    params = {
        'weight': 'best.pt',  # 模型权重文件路径
        'device': 'cuda:0',
        'method': 'GradCAMPlusPlus',  # GradCAMPlusPlus, GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM
        'layer': [10, 12, 14, 16, 18],  # 输入检测头的层
        'backward_type': 'all',  # detect:<class, box, all> obb:<box, angle, all> classify:<all>
        'conf_threshold': 0.2,  # 0.2
        'ratio': 0.02,  # 0.02-0.1
        'show_result': True,  # 不需要绘制结果请设置为False
        'renormalize': True,  # 把热力图限制在框内(仅对detect,segment,pose有效)
...
if __name__ == '__main__':
    ...
    # img_path可以是单张图片，也可以是文件夹
    # save_path：建议改成模型名称加heatmap_result
    model(r'C:\yolo11\datasets\exdark\test\images\2015_00026.jpg',
          'ELS-YOLO-GradCAMPlusPlus')

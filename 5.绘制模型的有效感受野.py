...
def get_params():
    params = {
        'weight': 'yolov8m.pt',  # 指定模型权重（训练好的.pt文件）
        'device': 'cuda:0',
        'layer': '9',  # 层数，一般选主干网络最后一层也就是SPPF层，也可以看其他的（从0开始，对照yaml文件）
        'dataset': r'C:\ultralytics-main\datasets\VisDrone2019\VisDrone2019-DET-train\images',  # 数据集路径
        'num_images': 50,  # 用多少张图片
        'save_path': 'erf_result/yolov8m_result.png'  # 保存的图片路径，以模型名称加png命名
    }
    return params

...
# 新
from ultralytics.nn.extra_modules.SoftEdgeConcat import *
...
from ultralytics.nn.modules import (
    ...
    SpatialAttention,
    CBAM,
    A2C2f, Silence, SilenceChannel, ChannelToNumber, NumberToChannel,
)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # --------------ASF--------------
            ...
            # c1：输入通道数，列表  c2：输出通道数（需要计算return，第二个特征图的通道数 * n）
            # args：yaml中传入模块的参数，列表，需要在这里重新编辑，添加通道数等信息
            c1 = [ch[x] for x in f]
            c2 = 3 * c1[1]
            args = [c1]
        elif m in {SoftEdgeConcat}:
            c1 = [ch[x] for x in f]
            c2 = 2 * c1[1]
            args = [c1, *args]
        elif m in {SBA}:

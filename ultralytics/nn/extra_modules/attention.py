...
# EMA (Exponential Moving Average)：一种基于指数加权移动平均的通道注意力机制。
# SimAM (Simple Attention Mechanism)：一种通过简单操作计算通道注意力的方法。
# EfficientChannelAttention：通过计算通道间的依赖关系为每个通道赋权的注意力机制。
# SEAttention (Squeeze-and-Excitation Attention)：通过压缩操作来生成通道注意力权重。
# CPCA (Channel-wise Principal Component Analysis Attention)：基于通道PCA分析的通道注意力机制。
# MPCA (Multi-channel PCA Attention)：基于多通道PCA的注意力机制。
# BAMBlock (Bottleneck Attention Module)：结合了通道注意力和空间注意力的混合模块，其中也涉及通道注意力。
# MLCA (Multi-level Channel Attention)：多级通道注意力机制。
# LSKA (Lightweight Spatial-Channel Attention)：同时处理通道和空间信息的注意力机制，但其中的通道部分是关键。

# 空间注意力（Spatial Attention）：
# SpatialGroupEnhance：增强空间特征的注意力机制。
# TripletAttention：基于三元组结构的空间注意力机制。
# CoordAtt (Coordinate Attention)：基于空间坐标生成空间注意力。
# SegNext_Attention：通过空间特征增强来提高图像分割性能的空间注意力机制。
# LocalWindowAttention：在局部窗口内进行空间注意力计算的机制。
# DAttention：增强空间关系的注意力机制。
# FocusedLinearAttention：通过空间上的线性加权来实现空间关注。
# AttentionTSSA：空间时间自注意力机制，处理空间特征与时间特征的关系。

# 混合注意力（Hybrid Attention）：
# BiLevelRoutingAttention：同时处理空间和通道的注意力机制，具有两级路由机制。
# BiLevelRoutingAttention_nchw：与 BiLevelRoutingAttention 类似，但在 NCHW 格式下工作。
# EfficientAttention：结合了空间和通道特征的高效注意力机制。
# LSKBlock (Lightweight Spatial-Kernel Block)：涉及到空间与通道的混合注意力计算。
# SegNext_Attention：融合空间与通道特征来优化图像分割的注意力机制。
# DualDomainSelectionMechanism：通过双重领域选择机制增强通道与空间之间的交互。

# 自注意力（Self-Attention）：
# TransNeXt_AggregatedAttention：通过自注意力机制来增强模型表示。
# AFGCAttention：自适应门控通道注意力机制，主要用于自注意力任务。

# 其他：
# EffectiveSEModule：与 SE (Squeeze-and-Excitation) 注意力相关的模块，主要涉及通道信息处理。
# CoordAtt：尽管它主要与空间特征关联，但它也在一定程度上涉及到通道信息，属于通道-空间混合类。
...
class EfficientChannelAttention(nn.Module):           # Efficient Channel Attention module
    ...
    def forward(self, x):
        ...
        out = self.avg_pool(x)
        # 把维度从 [B, C, 1, 1] → [B, 1, C]
        # 用一维卷积在通道维上建模 → 再变回原形状 [B, C, 1, 1]
        out = self.conv1(out.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        # 得到每个通道的权重（介于 0 和 1）
        out = self.sigmoid(out)
        # 把权重乘回原特征图，让重要通道变强、不重要的通道变弱
        return out * x

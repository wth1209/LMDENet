...
from ..modules.conv import Conv, DWConv, DSConv, RepConv, GhostConv, autopad, CBAM
...
from .hcfnet import PPA, LocalGlobalAttention, ECA
...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'CSP_MSCB_SC', 'EUCB_SC', 'C3k2_FAT', 'A2C2f_FMFFN', 'A2C2f_FMFFN_DYT', 'C3k2_LEGM', 'C3k2_RCB', 'DCEM',
           ]
...
class DCNV3_YOLO(nn.Module):
    ...
    def forward(self, x):
        # 如果定义了 stem_conv（即 inc ≠ ouc），则先进行通道调整
        if hasattr(self, 'stem_conv'):
            x = self.stem_conv(x)
        # 默认的特征图维度是 [N, C, H, W]，DCNv3 接收的输入是 [N, H, W, C]，因此要先做一次维度变换
        x = x.permute(0, 2, 3, 1)
        x = self.dcnv3(x)
        x = x.permute(0, 3, 1, 2)
...
class ContextGuideFusionModule(nn.Module):
    def __init__(self, inc) -> None:
        ...
        # # 调整通道数 + DCNv3
        # self.adjust_conv0 = DCNV3_YOLO(inc[0], inc[1], k=3)
        # self.adjust_conv1 = DCNV3_YOLO(inc[1], inc[1], k=3)
        ...
        # ------------注意力机制对比------------------
        # 有channel参数就需要传参（输出通道数）
        self.eca = EfficientChannelAttention(inc[1] * 2)
        # self.se = SEAttention(inc[1] * 2)
        # self.ema = EMA(inc[1] * 2)
        # self.cbam = CBAM(inc[1] * 2)
        # self.lsk = LSKBlock(inc[1] * 2)
        # self.simam = SimAM()
        # self.cpca = CPCA(inc[1] * 2)
        # self.lska = LSKA(inc[1] * 2, k_size=7)
        # self.mpca = MPCA(inc[1] * 2)
    ...
    def forward(self, x):
        ...
        # x_concat = self.cbam(x_concat)
        # x_concat = self.lsk(x_concat)
        # x_concat = self.simam(x_concat)
        # x_concat = self.cpca(x_concat)
        # x_concat = self.lska(x_concat)
        # x_concat = self.mpca(x_concat)
        ...
        # 将注意力机制加权后的特征图当作权重（利用注意力增强后的特征图彼此引导融合）
        x0_w = x0 * x0_weight
        x1_w = x1 * x1_weight

        # --------------拼接方式对比（要改tasks.py的c2参数）------------------
        # # 双向融合效果
...
class DCEM(nn.Module):
    def __init__(self, inc):
        ...
        # 输入特征图通道数不匹配
        if inc[0] != inc[1]:
            # 用1*1的卷积调整通道数，第一个特征图通道数变成第二个特征图通道数
            self.adjust_conv = Conv(inc[0], inc[1], k=1)
    ...
    def forward(self, x):
        ...
        # 将第一个特征图的通道数变成第二个特征图的通道数
        Fr = self.adjust_conv(Fr)

        # 在通道维度拼接
        x_concat = torch.cat([Fr, Fir], dim=1)  # n c h w

        x_concat = self.eca(x_concat)

        # 分成前后两份
        x0_weight, x1_weight = torch.split(x_concat, [Fr.size()[1], Fir.size()[1]], dim=1)
        ...
        w_r  = self.act(self.gap(D_r))    # [B, C, 1, 1]
        w_ir = self.act(self.gap(D_ir))   # [B, C, 1, 1]

        Fr_p  = Fr * w_r
        Fir_p = Fir * w_ir

        return torch.cat([Fr_p, Fir_p, com_fea], dim=1)
# ---------------------- 差分互补增强模块 end ----------------------

...
class ECA(nn.Module):
    # in_channel：输入通道数  gamma 和 b 用于自适应卷积核大小计算
    def __init__(self, in_channel, gamma=2, b=1):
        super(ECA, self).__init__()

        # 依据通道数 in_channel 自适应地确定1D卷积核大小 kernel_size。
        # 保证卷积核大小为奇数（使 padding 对称）
        k = int(abs((math.log(in_channel, 2)+b)/gamma))
        kernel_size = k if k % 2 else k+1
        padding = kernel_size//2

        # 全局平均池化，压缩空间维度，变成 [N, C, 1, 1]
        self.pool = nn.AdaptiveAvgPool2d(output_size=1)

        # 利用 Conv1d 在通道维上滑动建模局部通道间依赖关系；
        # Sigmoid 产生每个通道的注意力权重，范围在 [0,1]
    ...
    def forward(self, x):
        out = self.pool(x)  # shape: [N, C, 1, 1]
        out = out.view(x.size(0), 1, x.size(1))  # shape: [N, 1, C]
        out = self.conv(out)  # shape: [N, 1, C]
        out = out.view(x.size(0), x.size(1), 1, 1)  # shape: [N, C, 1, 1]
        return out * x

...
# 训练好的单模态模型的权重文件
model = YOLO("runs/train/DV对比试验/DV-T-yolo11n/weights/best.pt")

# source：数据地址(可以是文件夹、视频)
result = model.predict(source="datasets/DroneVehicle(RGB-T3-T)/train/infrared/images/00001.jpg",
                       imgsz=640,
                       show=False,
                       save_frames=True,
                       save=True,  # 保存结果
                       ...
                       name="DV-T3-yolo11n-train-infrared-00001",  # 模型名称加图片编号
                       # conf=0.2,  # 置信度
                       # iou=0.7,  # 交并比
